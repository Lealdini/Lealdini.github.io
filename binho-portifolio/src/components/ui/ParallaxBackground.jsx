...
const ParallaxBackground = () => {
  ...
  const [animationsEnabled, setAnimationsEnabled] = useState(false); // Desligado por padrão

  useEffect(() => {
    if (typeof window === 'undefined') return undefined;

    // Media query para detectar desktop (com mouse)
    ...
    // Listener para um toggle global de animações
    const handleToggle = (e) => setAnimationsEnabled(e.detail.enabled);
    window.addEventListener('binho:toggle-animations', handleToggle);

    return () => {
      if (mql.removeEventListener) mql.removeEventListener('change', apply);
      else mql.removeListener(apply);
      window.removeEventListener('binho:toggle-animations', handleToggle);
    };
  }, []);
  ...
  useEffect(() => {
    if (prefersReducedMotion || !isDesktop || !animationsEnabled) return undefined;
  ...
  }, [mouseX, mouseY, prefersReducedMotion, isDesktop, animationsEnabled]);

  // Partículas: 18 no desktop, 0 em mobile/reduced motion
  const particleCount = !prefersReducedMotion && isDesktop && animationsEnabled ? 18 : 0;
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden bg-background pointer-events-none">
      {/* Vinheta sutil top/bottom — sempre presente, mas leve */}
      <div className="absolute inset-0 bg-gradient-to-b from-background via-transparent to-background opacity-90" />

      {/* Efeitos pesados (glows, SVG, partículas, símbolos) só renderizam se as animações estiverem ativadas */}
      {animationsEnabled && (
        <>
          {/* DEEP — glow ambiente */}
          <motion.div
